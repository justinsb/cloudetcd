...
- **Leases**: A lease is persisted as an ordinary log record under an internal key (a grant is a put carrying the TTL, a revoke or expiry a delete), so replay rebuilds the lease table (a replayed lease starts with its full TTL again, as in etcd) and compaction keeps a grant exactly as long as the lease lives. The lease manager periodically expires leases and writes the corresponding `DELETE` operations to the log; internal keys are hidden from List results and Watch events.
